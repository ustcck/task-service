<template>
  <div>
    <h2 id="page-heading" data-cy="TaskHeading">
      <span v-text="$t('taskServiceApp.task.home.title')" id="task-heading">Tasks</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('taskServiceApp.task.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link
          :to="{ name: 'TaskCreate' }"
          tag="button"
        ...
        >
          ...
          <span v-text="$t('taskServiceApp.task.home.createLabel')"> Create a new Task </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && tasks && tasks.length === 0">
      <span v-text="$t('taskServiceApp.task.home.notFound')">No tasks found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="tasks">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('taskServiceApp.task.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('type')">
              <span v-text="$t('taskServiceApp.task.type')">Type</span>
            ...
            <th scope="row" v-on:click="changeOrder('level')">
              <span v-text="$t('taskServiceApp.task.level')">Level</span>
            ...
            <th scope="row" v-on:click="changeOrder('createTime')">
              <span v-text="$t('taskServiceApp.task.createTime')">Create Time</span>
            ...
            <th scope="row" v-on:click="changeOrder('updateTime')">
              <span v-text="$t('taskServiceApp.task.updateTime')">Update Time</span>
            ...
            <th scope="row" v-on:click="changeOrder('delFlag')">
              <span v-text="$t('taskServiceApp.task.delFlag')">Del Flag</span>
        ...
        <tbody>
          <tr v-for="task in tasks" :key="task.id" data-cy="entityTable">
            ...
            <td>{{ task.type }}</td>
            <td>{{ task.level }}</td>
            <td>{{ task.createTime ? $d(Date.parse(task.createTime), 'short') : '' }}</td>
            <td>{{ task.updateTime ? $d(Date.parse(task.updateTime), 'short') : '' }}</td>
            <td>{{ task.delFlag }}</td>
            <td class="text-right">
              <div class="btn-group">
                <router-link
                  :to="{ name: 'TaskView', params: { taskId: task.id } }"
                  tag="button"
                ...
                <router-link
                  :to="{ name: 'TaskEdit', params: { taskId: task.id } }"
                  tag="button"
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="taskServiceApp.task.delete.question" data-cy="taskDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-task-heading" v-text="$t('taskServiceApp.task.delete.question', { id: removeId })">
